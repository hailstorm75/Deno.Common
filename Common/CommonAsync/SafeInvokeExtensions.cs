...
namespace Common.Async
{
  public static class SafeInvokeExtensions
  {
    ...
    private static int MainThreadId => _locker.ReadLock(() => _mainThreadId ?? -1);
    ...
    public static void InitializeWithCurrentThread() => _locker.WriteLock(() => _mainThreadId = Thread.CurrentThread.ManagedThreadId);
    ...
    public static Task<TOut> SafeInvokeToMainThreadAsync<T, TOut>(this T target, Func<TOut> method)
      where T : ISynchronizeInvoke
    {
      return target.SafeInvokeAsync(() => Thread.CurrentThread.ManagedThreadId != MainThreadId ? default(TOut) : method());
    }
    ...
    public static Task<TOut> SafeInvokeToMainThreadAsync<T, TOut>(this T target, Func<Task<TOut>> taskFactory)
      where T : ISynchronizeInvoke
    {
      return target.SafeInvokeAsync(async () => Thread.CurrentThread.ManagedThreadId != MainThreadId ? default(TOut) : await taskFactory());
    }
