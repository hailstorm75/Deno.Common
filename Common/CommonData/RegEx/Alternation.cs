...
namespace Common.Data.RegEx
{
		...
		: RegularExpression, ICanSimplify
	{
		...
		public Alternation(IEnumerable<RegularExpression> parts)
			=> Parts = Flatten(parts).OrderBy(x => x.Length).Distinct().ToList();
		...
		private static RegularExpression ReduceLeft(Alternation part)
		{
			...
			var reduction = new Conjunction(new Literal(prefix), new Alternation(filtered.Select(x => x.Item1)).Simplify())
				  .FromSingleItem()
				  .Union(regExp.Where(x => !x.Key)
											 .SelectMany(x => x)
											 .Union(withLiterals.Select(x => x.Item1)));
		...
		private static RegularExpression ReduceRight(Alternation part)
		{
			...
			for (var i = 0; i < filtered.Count; i++)
			{
				...
				withLiterals.Remove(item);
				if (item.Item1 is IReduceable reduceable)
					item.Item1 = reduceable.ReduceRight(suffix);
				else
					throw new Exception("Invalid type.");
			}

			var reduction = new Conjunction(new Literal(suffix), new Alternation(filtered.Select(x => x.Item1)).Simplify())
				  .FromSingleItem()
				  .Union(regExp.Where(x => !x.Key)
											 .SelectMany(x => x)
											 .Union(withLiterals.Select(x => x.Item1)));
